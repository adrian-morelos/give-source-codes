...
if ( class_exists( 'Give_Recurring_Gateway' ) ) {
	...
	class Give_Recurring_PayPal_Pro_REST extends Give_Recurring_Gateway {
		...
		public function create_payment_profiles() {
			...
			$subscription_name = give_recurring_generate_subscription_name( $this->subscriptions['id'], $this->subscriptions['price_id'] );
			...
			$plan_details = array(
				...
				'amount'            => $this->subscriptions['recurring_amount'],
			...
			$renewal_date = date( 'c', strtotime( date( 'Y-m-d H:i:s' ) . ' +1 ' . $this->subscriptions['period'] ) );
		...
		public function generate_plan_id( $subscription_name ) {
			...
			return sanitize_key( $subscription_name . '_' . $this->subscriptions['recurring_amount'] . '_' . $this->subscriptions['period'] . '_' . $this->subscriptions['bill_times'] );
		}
		...
		public function create_billing_plan( $args = array() ) {
			...
			$plan_name = give_recurring_generate_subscription_name( $this->subscriptions['id'], $this->subscriptions['price_id'] );
			...
			if ( isset( $error_obj->name ) ) {
				...
				switch ( $error_obj->name ) {
					...
					case 'VALIDATION_ERROR':
						...
						$error_message = __( 'One or more fields are invalid in the request.', 'give-recurring' );
						break;
				}
			}
