...
class Give_Recurring_PayPal extends Give_Recurring_Gateway {
	...
	public function process_paypal_subscr_payment( $ipn_data ) {
		...
		// Look to see if payment is same day as signup and we haven't set the transaction ID
		// on the parent payment yet.
	...
	public function can_cancel( $ret, $subscription ) {
		...
		//Check gateway
		if ( $subscription->gateway === $this->id && ! empty( $subscription->profile_id ) && false !== strpos( $subscription->profile_id, 'I-' ) && 'active' === $subscription->status ) {
			$ret = true;
		}
	...
	public function cancel_paypal_standard( $subscription, $valid ) {
		...
		if ( empty( $success ) ) {
			wp_die( sprintf( __( 'There was a problem cancelling the subscription, please contact customer support. Error: %s', 'give-recurring' ), $error_msg ), array( 'response' => 400 ) );
		}
	 ...
	 *
	 * Adds the PayPal Standard settings to the Payment Gateways section (CMB2)
